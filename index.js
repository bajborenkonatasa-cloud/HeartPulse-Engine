...
const JOURNAL_ARCHIVE_KEEP = 200;
const NPC_KEEP = 40;
...
const KINK_LIBRARY = [
  ...
  ['phone','Секс по телефону / голос'], ['sexting','Секстинг'], ['somnophilia','Сомнофилия'], ['audio','Голос / аудио'], ['slowburn','Slow burn'], ['aftercare','Aftercare'],
...
function safeParse(s){
  ...
  // LLMs sometimes emit JSON-like numeric deltas as +7 / +2. Strict JSON rejects a leading '+'.
  // Normalize only numeric values that occur after ':', ',' or '['; leave quoted text untouched.
  try{
    const normalized=String(s).replace(/([:\[,]\s*)\+(?=\d)/g,'$1');
...
function stateMerge(raw){
  ...
  for(const key of ['kinks','activeKinks','customKinks','intentions','npc','journal','journalArchive','activeFeelings']) if(!Array.isArray(s[key])) s[key]=[];
...
function compactHistory(s){
  if(!Array.isArray(s.journalArchive)) s.journalArchive=[];
  if(Array.isArray(s.journal) && s.journal.length>JOURNAL_KEEP){
    const moved=s.journal.slice(JOURNAL_KEEP);
    s.journalArchive=[...moved,...s.journalArchive].slice(0,JOURNAL_ARCHIVE_KEEP);
    s.journalArchiveCount += moved.length;
    s.journal=s.journal.slice(0,JOURNAL_KEEP);
  }
  if(s.journalArchive.length>JOURNAL_ARCHIVE_KEEP) s.journalArchive=s.journalArchive.slice(0,JOURNAL_ARCHIVE_KEEP);
...
function visibleFeelingKeys(s){
  // Keep the model-selected salient feelings first, but always fill the Pulse
  // to six slots when possible. This is display-only: it does not change the
  // parser, transport, stored relation values, or activeFeelings semantics.
  const requested=(Array.isArray(s.activeFeelings)?s.activeFeelings:[]).filter(k=>REL_LABEL[k]);
  const ranked=REL_KEYS.map(k=>[k,clamp(s.relation[k],0,REL_MAX)])
    .filter(([,v])=>v>0)
    .sort((a,b)=>b[1]-a[1])
    .map(([k])=>k);
  return [...new Set([...requested,...ranked,...DEFAULT_VISIBLE_REL])].slice(0,6);
}
...
function buildPrompt({includeAutoSpark=false}={}){
  ...
  if(Array.isArray(s.npc) && s.npc.length){
    const npcMemory=[...s.npc].sort((a,b)=>(b.lastSeen||0)-(a.lastSeen||0)).slice(0,8).map(n=>{
      const rel=n.relation||{};
      const bits=[n.state&&`state=${n.state}`,n.mood&&`mood=${n.mood}`,n.motive&&`motive=${n.motive}`,n.goal&&`goal=${n.goal}`].filter(Boolean);
      const relBits=['trust','affection','desire','irritation','fear','respect'].filter(k=>Number(rel[k])>0).map(k=>`${k}:${clamp(rel[k],0,REL_MAX)}`);
      return `${n.name}${bits.length?` {${bits.join(' | ')}}`:''}${relBits.length?` [${relBits.join(',')}]`:''}`;
    });
    blocks.push(`[NPC MEMORY] Known recurring NPCs: ${npcMemory.join('; ')}. Preserve their continuity. If one appears, is named by {{user}}, speaks, acts, is discussed as part of the current conflict/relationship, or becomes materially relevant, include that NPC in the HeartPulse npc packet even if their state changed only slightly.`);
  }
  if(s.injectKinks){
  ...
  blocks.push(`[VISIBLE OUTPUT RULE] Do not print any HeartPulse status panel, relationship percentages, motives/goals summary, or explanatory metadata in the visible roleplay prose. HeartPulse will intercept the service packet before the message is rendered.`);
  if(s.autoTrack){
    ...
    blocks.push(`[HP UPDATE] At the VERY END of the reply append exactly ONE machine packet using these literal boundary lines:
[[HEARTPULSE_STATE]]
{"label":"...","shift":"...","active":["love","tenderness"],"note":"1–2 short sentences explaining what is emotionally strongest right now","levels":null,"deltas":{},"inner":{"mood":null,"motives":null,"hiddenThought":null,"currentGoal":null,"futureDesire":null},"intentions_add":[],"intentions_done":[],"npc":[{"name":"...","state":"...","mood":"...","motive":"...","goal":"...","relation":{"trust":0,"affection":0,"desire":0,"irritation":0,"fear":0,"respect":0}}]}
[[/HEARTPULSE_STATE]]
Do NOT wrap this packet in a code fence, XML, HTML comment, markdown, or prose. Do not put anything after the closing boundary. HeartPulse removes this packet locally before normal rendering. ${calibration} Keep text fields compact and in Russian. NPC RULE: the npc array is mandatory whenever a named non-main character appears in the assistant reply OR is named/acted with in the user's immediately preceding message OR is materially relevant to the current relationship/conflict. Include every such named NPC, not only newly introduced ones. Preserve/update known NPCs from [NPC MEMORY]. Do not omit an active NPC merely because their state changed only slightly. Exclude generic unnamed people/background extras.
...
function normalizeNpc(n,old={}){
  ...
  const relation=Object.assign({trust:0,affection:0,desire:0,irritation:0,fear:0,respect:0},old.relation||{});
  if(n?.relation&&typeof n.relation==='object') for(const k of relKeys) if(k in n.relation) relation[k]=clamp(n.relation[k],0,REL_MAX);
  return {id:String(n?.id||old.id||makeId()),name:String(n?.name||old.name||'NPC').slice(0,80),state:String(n?.state||n?.note||old.state||'').slice(0,240),mood:String(n?.mood||old.mood||'').slice(0,180),motive:String(n?.motive||old.motive||'').slice(0,220),goal:String(n?.goal||old.goal||'').slice(0,220),relation,lastSeen:now()};
}
...
function findHeartPulsePacket(text){
  ...
  // v0.9.9 primary transport: explicit service boundaries. This is intentionally
  // easy to parse, then removed from the message before CHARACTER_MESSAGE_RENDERED.
  const open='[[HEARTPULSE_STATE]]';
  const close='[[/HEARTPULSE_STATE]]';
  const pos=text.lastIndexOf(open);
  if(pos>=0){
    const bodyStart=pos+open.length;
    const closePos=text.indexOf(close,bodyStart);
    if(closePos>=0){
      const raw=text.slice(bodyStart,closePos).trim();
      const bal=extractBalancedJson(raw,0);
      const json=bal?.json || raw;
      const packet=safeParse(json);
  ...
  const marker='<!--HEARTPULSE_STATE:';
  const legacyPos=text.lastIndexOf(marker);
  if(legacyPos<0) return null;
  const bal=extractBalancedJson(text,legacyPos+marker.length);
  if(!bal) return null;
  const suffix='-->';
  const sx=text.indexOf(suffix,bal.end);
  if(sx<0) return null;
  const packet=safeParse(bal.json);
  if(!packet) return {packet:null,source:'html-comment-invalid',start:legacyPos,end:sx+suffix.length,raw:bal.json};
  return {packet,source:'html-comment-legacy',start:legacyPos,end:sx+suffix.length,raw:bal.json};
}

async function parseModelStateAtIndex(index,reason='event'){
  ...
  index=Number(index);
  const last=Number.isInteger(index) ? c.chat[index] : null;
  if(!last || last.is_user || typeof last.mes!=='string'){
    d.lastParseStatus=`Нет подходящего ответа ассистента (${reason})`;
    d.lastParseError='';
  ...
  let found=null;
  try{ found=findHeartPulsePacket(last.mes); }
  catch(e){
    d.lastParseStatus='Ошибка разбора пакета';
    d.lastParseError=String(e?.message||e);
    d.lastPacketSource='parser-exception';
    await persistExactState(s); console.warn('[HeartPulse] packet parser failed',e); renderModelPreview(); return false;
  }
  ...
  try{
    ...
    d.lastParseStatus='Пакет найден, но применение завершилось ошибкой';
    d.lastParseError=String(e?.message||e);
    d.lastPacketSource=found.source;
    await persistExactState(s); console.error('[HeartPulse] packet apply failed',e); renderModelPreview(); return false;
  }
}
...
async function parseLatestModelState(reason='event'){
  ...
  if(!indexed){
    const s=getState(); s.diagnostics=Object.assign({},defaults().diagnostics,s.diagnostics||{});
...
function panelHtml(){
  ...
  const journalEntries=(s.journal||[]).map((j,i)=>Object.assign({_index:i},j));
  const journalShown=ui.showAllJournal?journalEntries:journalEntries.slice(0,10);
  const journalGroups=[];
  for(const item of journalShown){
    const day=new Date(item.ts||0).toLocaleDateString();
    let group=journalGroups.find(g=>g.day===day);
    if(!group){ group={day,items:[]}; journalGroups.push(group); }
  ...
  return `<div id="hpOverlay" class="hp-overlay hp-hidden"><div id="hpPanel" class="hp-panel">
    ...
    <main class="hp-body">
      ...
      ${page('journal',`<div class="hp-soft-card hp-journal-card"><div class="hp-journal-title"><h3>📜 Журнал сдвигов</h3><span class="hp-muted hp-micro">${(s.journal||[]).length} активных · ${(s.journalArchive||[]).length} в архиве</span></div><div class="hp-journal-tools"><button id="hpJournalToggle" type="button">${ui.showAllJournal?'Показать последние 10':'Показать весь журнал'}</button><button id="hpJournalMerge" type="button">🧩 Объединить выбранные</button><button id="hpJournalArchiveOld" type="button">🗃️ Архивировать старые</button></div>${journalHtml||'<p class="hp-muted">Журнал пока пуст.</p>'}${(s.journalArchive||[]).length?`<details class="hp-journal-archive"><summary>🗃️ Архив (${s.journalArchive.length})</summary><div class="hp-journal-tools"><button id="hpJournalClearArchive" type="button">Очистить архив</button></div>${s.journalArchive.slice(0,60).map(j=>`<div class="hp-log hp-log-archived"><time>${new Date(j.ts).toLocaleString()}</time><span>${esc(j.text)}</span></div>`).join('')}</details>`:''}${s.journalArchiveCount?`<p class="hp-muted hp-micro">Всего когда-либо архивировано/свёрнуто: ${s.journalArchiveCount}.</p>`:''}</div>`)}
...
function bind(){
  ...
  q('#hpJournalToggle')?.addEventListener('click',()=>{ const ui=readUi(); ui.showAllJournal=!ui.showAllJournal; saveUi(ui); render(); });
  document.querySelectorAll('[data-journal-edit]').forEach(b=>b.addEventListener('click',async()=>{ const s=getState(),idx=Number(b.dataset.journalEdit),j=s.journal[idx]; if(!j)return; const text=window.prompt('Изменить запись журнала:',String(j.text||'')); if(text===null)return; const clean=text.trim(); if(!clean)return; j.text=clean; await saveState(); render(); }));
  document.querySelectorAll('[data-journal-del]').forEach(b=>b.addEventListener('click',async()=>{ const s=getState(),idx=Number(b.dataset.journalDel),j=s.journal[idx]; if(!j)return; if(!window.confirm('Удалить эту запись из журнала?'))return; s.journal.splice(idx,1); await saveState(); render(); }));
  q('#hpJournalMerge')?.addEventListener('click',async()=>{ const selected=[...document.querySelectorAll('[data-journal-select]:checked')].map(x=>Number(x.dataset.journalSelect)).filter(Number.isInteger).sort((a,b)=>a-b); if(selected.length<2)return toast('Выбери минимум две записи','info'); const s=getState(),items=selected.map(i=>s.journal[i]).filter(Boolean); if(items.length<2)return; const merged=[...new Set(items.map(x=>String(x.text||'').trim()).filter(Boolean))].join(' · '); const newest=Math.max(...items.map(x=>Number(x.ts)||0)); selected.sort((a,b)=>b-a).forEach(i=>s.journal.splice(i,1)); s.journal.unshift({ts:newest||now(),type:'merged',text:merged}); await saveState(); toast(`Объединено записей: ${items.length}`,'success'); render(); });
  q('#hpJournalArchiveOld')?.addEventListener('click',async()=>{ const s=getState(); if((s.journal||[]).length<=10)return toast('Старых записей для архива пока нет','info'); const moved=s.journal.splice(10); s.journalArchive=[...moved,...(s.journalArchive||[])].slice(0,JOURNAL_ARCHIVE_KEEP); s.journalArchiveCount=(Number(s.journalArchiveCount)||0)+moved.length; await saveState(); toast(`В архив перенесено: ${moved.length}`,'success'); render(); });
  q('#hpJournalClearArchive')?.addEventListener('click',async()=>{ const s=getState(); if(!(s.journalArchive||[]).length)return; if(!window.confirm('Очистить архив журнала? Текущие чувства, цели и NPC не будут удалены.'))return; s.journalArchive=[]; await saveState(); toast('Архив журнала очищен','success'); render(); });
  ...
  const clearNpcForm=()=>{ const ids=['hpNpcEditId','hpNpcName','hpNpcState','hpNpcMood','hpNpcMotive','hpNpcGoal']; ids.forEach(id=>{const el=q('#'+id);if(el)el.value='';}); for(const k of ['trust','affection','desire','irritation','fear','respect']){const el=q('#hpNpcRel_'+k);if(el)el.value=0;} };
  q('#hpNpcSave')?.addEventListener('click',async()=>{
    const name=q('#hpNpcName')?.value?.trim(); if(!name) return toast('Напиши имя NPC','info');
    const s=getState(), editId=q('#hpNpcEditId')?.value||'', old=(s.npc||[]).find(n=>n.id===editId)||{};
    const relation={}; for(const k of ['trust','affection','desire','irritation','fear','respect']) relation[k]=clamp(q('#hpNpcRel_'+k)?.value||0,0,REL_MAX);
    const item=normalizeNpc({id:editId||old.id||makeId(),name,state:q('#hpNpcState')?.value?.trim()||'',mood:q('#hpNpcMood')?.value?.trim()||'',motive:q('#hpNpcMotive')?.value?.trim()||'',goal:q('#hpNpcGoal')?.value?.trim()||'',relation},old);
    const idx=(s.npc||[]).findIndex(n=>n.id===item.id); if(idx>=0)s.npc[idx]=item; else { const byName=(s.npc||[]).findIndex(n=>String(n.name||'').toLowerCase()===name.toLowerCase()); if(byName>=0)s.npc[byName]=normalizeNpc(item,s.npc[byName]); else s.npc.unshift(item); }
    await saveState(); toast(`NPC сохранён: ${name}`,'success'); render();
  });
  q('#hpNpcCancel')?.addEventListener('click',clearNpcForm);
  document.querySelectorAll('[data-npc-edit]').forEach(b=>b.addEventListener('click',()=>{ const s=getState(),n=(s.npc||[]).find(x=>x.id===b.dataset.npcEdit); if(!n)return; q('#hpNpcEditId').value=n.id||''; q('#hpNpcName').value=n.name||''; q('#hpNpcState').value=n.state||''; q('#hpNpcMood').value=n.mood||''; q('#hpNpcMotive').value=n.motive||''; q('#hpNpcGoal').value=n.goal||''; for(const k of ['trust','affection','desire','irritation','fear','respect']) q('#hpNpcRel_'+k).value=clamp(n.relation?.[k]||0,0,REL_MAX); q('#hpNpcName')?.scrollIntoView({behavior:'smooth',block:'center'}); }));
  document.querySelectorAll('[data-npc-del]').forEach(b=>b.addEventListener('click',async()=>{ const s=getState(),n=(s.npc||[]).find(x=>x.id===b.dataset.npcDel); if(!n)return; if(!window.confirm(`Удалить NPC «${n.name}» из памяти HeartPulse?`))return; s.npc=s.npc.filter(x=>x.id!==b.dataset.npcDel); await saveState(); toast(`NPC удалён: ${n.name}`,'success'); render(); }));
